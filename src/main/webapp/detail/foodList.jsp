...
<script type="text/javascript">
	function confirmDelete(foodId) {
		if(confirm("是否删除该菜品")){
			window.location="deleteFood/"+foodId;
		}
	}
        ...
        <tbody id="TableData">
			<c:forEach var="list" items="${requestScope.list.data}">
				<tr class="TableDetail1">
					...
					<td><a href="${pageContext.request.contextPath}/queryFoodDatail/${pageScope.list.foodid}">${pageScope.list.foodname}&nbsp;</a></td>
					<td>${pageScope.list.price}&nbsp;</td>
					<td>
						<a href="${pageContext.request.contextPath }/detail/updateFood.jsp?foodId=${pageScope.list.foodid}&foodName=${pageScope.list.foodname}&price=${pageScope.list.price}&path=${pageScope.list.path}"  
        		...
        		<tr>
					<td colspan="3">
						...
						<a href="${pageContext.request.contextPath}/showFood?curPage=${requestScope.list.prePage}">上一页</a>
						...
						<a href="${pageContext.request.contextPath}/showFood?curPage=${requestScope.list.nextPage}">下一页</a>
						<a href="${pageContext.request.contextPath}/showFood?curPage=${requestScope.list.totalPage}">尾页</a>
					</td>
					<td>每页显示${requestScope.list.pageCount}条数据,共${requestScope.list.totalPage}页</td>
				</tr>
        </tbody>
    </table>
   <!-- 其他功能超链接 -->
